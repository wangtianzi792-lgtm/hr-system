...
def get_punch_records(
    ...
    """从 raw_punch_records 读取打卡数据"""
    from app.models.raw_punch import RawPunchRecord

    query = db.query(RawPunchRecord).join(Employee)
    ...
    if department_id:
        query = query.filter(Employee.department_id == department_id)
    if start_date:
        query = query.filter(func.date(RawPunchRecord.punch_time) >= start_date)
    if end_date:
        query = query.filter(func.date(RawPunchRecord.punch_time) <= end_date)
    ...
    for r in records:
        ...
        items.append({
            ...
            "punch_date": str(r.punch_time)[:10],
