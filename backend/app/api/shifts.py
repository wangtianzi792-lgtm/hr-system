...
def update_shift(shift_id: int, data: ShiftUpdate, db: Session = Depends(get_db)):
    ...
    for field, value in data.model_dump(exclude_unset=True).items():
        if field in ("check_in_start", "check_in_end", "check_out_start", "check_out_end") and value:
            setattr(shift, field, datetime.strptime(value, "%H:%M").time())
...
def get_employee_shifts(employee_id: int, db: Session = Depends(get_db)):
    """获取指定员工的排班和班次分配信息"""
    from app.models.employee import Employee
    ...
    # 排班记录
    schedules = db.query(ScheduleRecord).filter(
        ScheduleRecord.employee_id == employee_id
    ).order_by(ScheduleRecord.schedule_date.desc()).limit(30).all()
